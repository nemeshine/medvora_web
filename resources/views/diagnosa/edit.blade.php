...
<body>
  ...
  <div class="container-fluid">
    <div class="card-form">
      <div class="form-title">Edit Diagnosa Penyakit</div>
      <form method="POST" action="{{ route('diagnosa.update',$diagnosa) }}">
        @csrf @method('PUT')
          ...
          <select name="id_pasien" class="form-select @error('id_pasien') is-invalid @enderror">
            ...
            @foreach($pasiens as $pasien)
              <option value="{{ $pasien->id_pasien }}"
                {{ old('id_pasien',$diagnosa->id_pasien)==$pasien->id_pasien?'selected':'' }}>
                {{ $pasien->nama_pasien }}
              </option>
            @endforeach
          </select>
        ...
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <input type="date" name="tanggal_keluhan"
                   ...
                   value="{{ old('tanggal_keluhan',$diagnosa->tanggal_keluhan) }}">
          ...
          <div class="col-md-6">
            ...
            <input type="date" name="tanggal_diagnosa"
                   ...
                   value="{{ old('tanggal_diagnosa',$diagnosa->tanggal_diagnosa) }}">
        ...
        <div class="mb-3 mt-3">
          ...
          <textarea name="keluhan" rows="3"
                    class="form-control @error('keluhan') is-invalid @enderror">{{ old('keluhan',$diagnosa->keluhan) }}</textarea>
        ...
        <div class="mb-3">
          ...
          <input type="text" name="diagnosa"
                 ...
                 value="{{ old('diagnosa',$diagnosa->diagnosa) }}">
          @error('diagnosa')<div class="invalid-feedback">{{ $message }}</div>@enderror
        </div>

        <div class="mb-3">
          <label class="form-label">Resep Obat</label>
          <textarea name="resep_obat" rows="2" class="form-control">{{ old('resep_obat',$diagnosa->resep_obat) }}</textarea>
        </div>

        <div class="mb-3">
          <label class="form-label">Catatan Tambahan</label>
          <textarea name="catatan_tambahan" rows="2" class="form-control">{{ old('catatan_tambahan',$diagnosa->catatan_tambahan) }}</textarea>
        </div>
